...
export default function MonitoringPage() {
  ...
  const stressCommands = [
    {
      ...
      command: "for i in $(seq 1 $(nproc)); do yes > /dev/null & done; sleep 60; pkill -9 yes 2>/dev/null || killall yes 2>/dev/null",
    ...
    },
    {
      ...
      command: `python3 -c "import time; t=time.time()+60; [i*i for i in range(100000000) if time.time()<t]"`,
      description: "Allocates memory and runs floating-point iterations for 60s.",
    },
    {
      id: "stress-pkg",
      title: "Linux Stress Utility (60 Seconds)",
      command: "sudo apt update && sudo apt install -y stress && stress --cpu 2 --vm 1 --vm-bytes 256M --timeout 60s",
    ...
    {
      id: "stop-all",
      title: "Terminate All Active Stress Processes",
      command: "pkill -9 yes 2>/dev/null; pkill -9 -f 'while true' 2>/dev/null; pkill -9 python3 2>/dev/null; pkill -9 stress 2>/dev/null; killall yes 2>/dev/null",
  ...
  const powerCommands = [
    ...
    {
      id: "reboot-ssh",
      title: "Reboot EC2 Instance (Inside SSH - No AWS CLI Needed)",
      command: "sudo reboot",
      description: "Standard Linux command to reboot the VM directly from inside your SSH session.",
    },
    {
      id: "install-aws-cli",
      title: "Install AWS CLI on Ubuntu EC2 (Optional)",
      command: "sudo apt update && sudo apt install -y awscli && aws configure",
      description: "Run this inside SSH if you want to use the aws command line tool inside the server.",
    },
    {
      id: "stop-aws-cli",
      title: "Stop EC2 Instance (From Local PC via AWS CLI)",
    ...
    {
      id: "start-aws-cli",
      title: "Start EC2 Instance (From Local PC via AWS CLI)",
    ...
    {
      id: "reboot-aws-cli",
      title: "Reboot EC2 Instance (From Local PC via AWS CLI)",
    ...
    {
      id: "status-aws-cli",
      title: "Check State (From Local PC via AWS CLI)",
